...
fun main() {
    val testRobots = testData.map { it.split(" ", "p=", ",", "v=").filter { v -> v.isNotBlank() } }
        .map { Robot(Coord(it[0].toInt(), it[1].toInt()), Vect(it[2].toInt(), it[3].toInt())) }

    val realRobots = actualData.map { it.split(" ", "p=", ",", "v=").filter { v -> v.isNotBlank() } }
        .map { Robot(Coord(it[0].toInt(), it[1].toInt()), Vect(it[2].toInt(), it[3].toInt())) }
...
fun part1(robots: List<Robot>, w: Int, h: Int) {
    ...
    val q1 = state.filter { it.p.x < qx && it.p.y < qy }
    val q2 = state.filter { it.p.x > qx && it.p.y < qy }
    val q3 = state.filter { it.p.x < qx && it.p.y > qy }
