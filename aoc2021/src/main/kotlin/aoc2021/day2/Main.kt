...
fun main() {
    ...
    val result = input.fold(Pair(0, 0)) { acc, m ->
        when (m.direction) {
            "forward" -> Pair(acc.first + m.value, acc.second)
            "up" -> Pair(acc.first, acc.second - m.value)
            "down" -> Pair(acc.first, acc.second + m.value)
